...
export default function Cart() {
  ...
  return (
    <div className="flex flex-col w-full h-full justify-between">
      ...
      {cartItems.length > 0 ? (
        <Scrollbar className="cart-scrollbar w-full flex-grow">
          <div className="w-full px-5 md:px-7">
            {cartItems?.map((item) => (
              <CartItem item={item} key={item.DesignId} />
            ))}
          </div>
        </Scrollbar>
      ) : (
      ...
      >
        <Link
          href={cartItems ? ROUTES.CHECKOUT : "/"}
          className={
            cartItems.length > 0
              ...
              : "w-full px-5 py-3 md:py-4 flex items-center justify-center bg-heading rounded-md text-sm sm:text-base text-white focus:outline-none transition duration-300 hover:bg-gray-600 cursor-not-allowed bg-gray-400 hover:bg-gray-400"
          }
        >
